...
export class ApiMonitorService {
  ...
  private baseUrl = 'http://localhost:7070/api/monitor';

  getApis(): Observable<MonitoredApi[]> {
    return this.http.get<MonitoredApi[]>(this.baseUrl);
  }

  getLatestStatus(apiId: number): Observable<LatestStatusResponse> {
    return this.http.get<LatestStatusResponse>(
      `http://localhost:7070/api/history/${apiId}/latest`,
    );
  }

  getHistory(apiId: number): Observable<HistoryResponse[]> {
  ...
  updateApi(id: number, request: CreateApiRequest) {
    return this.http.put(`${this.baseUrl}/${id}`, request);
  }

  getApiById(id: number) {
    return this.http.get<MonitoredApi>(`${this.baseUrl}/${id}`);
  }
